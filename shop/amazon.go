...
var AmazonRegex = regexp.MustCompile(`(?i)\s*(((4G )?(Handys?|Smartphones?))( mit)?|ohne Vertragy?,?|(4G )?Outdoor|(\+\W*)Kopfhörer|Günstige?,?|Neu|Telekom|(IP\d+\s+)?Wasserdichi?t(er)?|\d+MP(\+8MP)?\W+(AI\W*)?(Dual\W+|Quad\W+|Unterwasser)?Kamera|Dual\W+SIM(\+SD \(.*?\))?|\d Zoll Touch Bildschirm,|EU 128GB|OTG Reverse Charge|Cloud Navy|Erweiterbar|Octa\W*Core(\W*Pro[cz]essor)?|Großer? Akku|(Starker )?(\d{4,5}|\d{1,3}\.\d{3})\s*mAh(\W*(Großer )?Akku)?|\b20[12]\d|\W*\d+(GB)?\s*\+\s*\d+\s*GB([\/+]\d+[GT]B)?\)?,?|Android \d+(\.\d)?|(SM )?[SG]\d{3}[A-Z]*)`)
...
func XXX_amazon(isDryRun bool) IShop {
	...
	const _url = "https://www.amazon.de/s?k=Simlockfreie+Handys&i=electronics&rh=n:15326400031,p_n_free_shipping_eligible:20943778031,p_n_deal_type:26902994031,p_n_condition-type:776949031&dc=&c=ts&qid=1678975426&rnid=776942031&page=%d"
	...
	for p := 1; p <= 10; p++ {
		...
		for item := productList.FirstChild.NextSibling; item != nil; item = item.NextSibling.NextSibling {
			...
			// title = strings.Split(strings.Split(title, " (")[0], " Handy")[0]
			if _debug {
				fmt.Println(title)
			}

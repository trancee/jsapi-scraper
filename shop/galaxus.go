...
var GalaxusCleanFn = func(name string) string {
	...
	name = regexp.MustCompile(`\s+[2345]G(\s+EU)?(\s+NE)?(\s+Phone)?|\s+I9505|\s+[A]\d{3}[B]| XT\d{4}-\d+|( Blackview| Graues)? Smartphone( Blackview| oppo)?| Smartfon|^Vodafone |^TIM |^TE Connectivity |OPP DS | Snapdragon| Black| 2 ”| MOBILE PHONE| SMARTPHONE MOTOROLA|Motorola Smartfon | Handy| OEM| TCT| VoLTE| \+ Huawei| Bluetooth Speaker| Limited| Telefon(as)?|( Porsche)? Design| czarny| pomarańczowy| zielony`).ReplaceAllString(name, "")

...
var GalaxusRegex = regexp.MustCompile(`, |\s+\d\/\d+|\s*\(?(\d+(GB)?[+\/])?\d+\s*GB\)?|\s+\(?20[12]\d\)?|\s+4g|\s+X\d{3}F|\s+\(V\d{4}\)|\d{4} mAh|\s+\(?\d+.\d+( Zoll| cm|\")\)?| DS\s*\d|\s+((EE )?Enterprise Edition( CH)?)| EU| LTE| NFC| (Dual|DUAL)[ -](Sim|SIM)|GREEN | Elegant Black| Force Touch| Grey| Midnight Space|Space Silver| bamboo green| midday dream| midnight blue`)
...
var GalaxusCleanFn = func(name string) string {
	...
	name = regexp.MustCompile(`\s+[2345]G(\s+EU)?(\s+NE)?|\s+I9505| XT\d{4}-\d+|( Blackview)? Smartphone| Snapdragon| Black| 2 ”`).ReplaceAllString(name, "")

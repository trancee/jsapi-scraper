...
var FolettiRegex = regexp.MustCompile(`(?i)\s*[-,]+\s+|\s*\(?(\d+(\s*GB)?[+\/])?\d+\s*GB\)?|\s*\d+G|(2|4|6|8|12)\/(64|128|256?B?)(GB)?|\s+\(?20[12]\d\)?|\s*\d+([,.]\d+)?\s*(cm|\")|\d{4,5}\s*mAh|\s+20[12]\d|\s+(Hybrid|Dual\W(SIM|Sim)|(EE )?Enterprise( Edition)?( CH)?|inkl\.|LTE|NFC|smartphone)|((Astro|Atlantic|Bamboo|Black|Blau|Blue|Charcoal|Cosmic|Dark|Denim|Diamond|Dusk|Electric|Elegant|Frosted|Glowing|Graphite|Gr[ae]y|Grau|Gravity|Himalaya|Ic[ey]|Lagoon|Lake|Lavender|Light|Matte|Metallic|Meteorite|Meteor|Midnight|Mint|Misty|Night|Ocean|Onyx|Petrol|Polar|Prism|Sage|Sandy|Schwarz|Silber|Sky|Stargaze|Steel|Sterling|Sunburst|Titanium|Titan|Twilight|Zeus)\b[\s\/]?)(Black|Blau|Blue|Gold|Green|Gr[ae]y|Grau|Onyx|Rose|Schwarz|Silber|White)?`)
...
var FolettiCleanFn = func(name string) string {
	...
	name = regexp.MustCompile(` XT\d{4}-\d+|SMARTPHONE\s*|Smartphone\s*|Smartfon\s*|Solutions |TIM | Mobility Motorola| Mobility| Outdoor| NE| EE|o2-Aktion |#GOECO`).ReplaceAllString(name, "")

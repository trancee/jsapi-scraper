...
var TuttiRegex = regexp.MustCompile(`(?i)[,-]? ?[\[(]?\b(2|4|6|8|12|16|32|54|64|65|128|250|256|265|512) ?([MG][BG]|BG|G|\+)[)\]]?( RAM)?|(\. )?\b(6|8|12|128|256)(GB)?\/(6|8|12|128|256)(GB)?\b|\/6\s+|\(\d\.\d "| \d"|[ (]?[45] ?G[)]?| GSM| LTE| inkl\. | mit | und | and more| [*|] | \([^\d]| \/|, |\/ | -|;`)
...
var TuttiCleanFn = func(name string) string {
	name = regexp.MustCompile(`(?i)^Original | - neuwertiger Zustand|^\*\*\*\*|^\*\*|^\+\+ | \+\+$| \+ Airpods| Entsperrt|^Für Einsteiger: |^Neu: |^Zu verkaufen: |Supper | Occass?ion|Gebrauchtes | Schnäppchen| GÜNSTIG|>BILLIG: |Hochwertiges |Cellulate |funktioniert (100%)?|bisschen kaputt aber geht noch gut| \d+%( Battery| di batteria)?|Batteriezustand|Batteriekapazität( \d+%)?|COMPLETO FUNZIONA BENISSIMO|(Perfetto |Ricondizionato )?Garanzia( n.134)?|RESERVIERT|(guter?|in (einem )?sehr gute[nm]|in exzellente[nm]|neuwertiger|Perfekter|(in )?Top) Zustand|ohne Kratzer|(voll )?funktionstüchtig|im sehr guten Zustand|mit Box|( - )?sehr guter Zustand|Hammer Geil| in ottimo stato|semplificato |Mobile Phone( - )?|Mobile?telefon (von )?| (Smart(fon|phone)|Handy)( - |:)?|Telefon(ino|o)?( per anziani)?|mobile |(leicht)? gebraucht( & gelöscht)?|(Micro-|Neuwertiges |Komfort-)?(Handy|Natel) (von |\/ |- )?|Neuwertiges |Handy\s*/\s*(Natel|Smart(fon|phone))( - |:)?|(zu )?verkaufen? ?(ein )?|Verkauf von |vendo | vendesi|preis verhandelbar|\(Tausche\/Verhandelbar\) |originalverpackt|neu & ungeöffnet|(in|inklusive|mit|NEU und| und)? OVP|( - )?(wie |Fabrik)?neu(es?|wertig)?|24 Monate Garantie| New Version| und noch verschweisst| nie benutzt| in gute[mn] Zustand| mit Gebrauchsspuren|einwandfrei|renoviert|4 Farben|Gratis ?versand|in Lederetui|mit Eingabestift|läuft einwandfrei| MIT GOOGLE SERVICES|Original OS|CalyxOS|GrapheneOS|(neues |Android )?Smart(fon|phon)e?( -|:)? |Burnerphone |Neuwertiges |in usato|(Usato ma )?(Praticamente )?Nuovo( |!)?| garandieschein| anno| année| con vetro da sostituire| \+ accessori| Glasschaden| HD\+|( - )?dual[ -]?sim|(\d\.)?\d Zoll|miui|Firmengerät| Apple| ESR| MagSafe| Original Taptic Engine| Original Front Kamera Module| Original Kamera Module| 4K Kamera| inkl\. Ladegerät| Gehäuse Original|Google Sperre|SIMLOCKED| Speicher| di Xiaomi|condizioni ottime| modell|cellulare |\[DANNEGGIATO\]|(12|16|32|64|128|256)\+(2|3|4|6|8|12|16)GB|(2|3|4|6|8|12|16)\s*ram|(16|32|64|128|256)\s*rom|512,$|Breathing Crystal|Red Product| RED$|Smaragdgrün| (Android|EU)$`).ReplaceAllString(name, "")

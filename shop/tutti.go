...
var TuttiRegex = regexp.MustCompile(`(?i)[,-]? ?(2|4|6|8|16|32|54|64|128|256|265) ?([MG]B|BG|G)( RAM)?|\/6\s+| \d"| [45] ?G| GSM| (ancora|(electric )?black|blau|chrome|gray|onyx|Pearl White|red edition|rose|(rose )?gold|nero|roségold|rosso|rot|schwarz|silber|silver|space gr[ae]y|weiss|white)| mit | und | [*|] | \(| \/|, |\/ | - `)
...
var TuttiCleanFn = func(name string) string {
	name = regexp.MustCompile(`(?i)^Original | Entsperrt| Occass?ion|Gebrauchtes | Schnäppchen| GÜNSTIG|Cellulate |funktioniert|COMPLETO FUNZIONA BENISSIMO|Garanzia|RESERVIERT|Top Zustand|funktionstüchtig|im sehr guten Zustand|mit Box|( - )?sehr guter Zustand| in ottimo stato|semplificato |Mobile Phone( - )?|Mobile?telefon | (Smart(fon|phone)|Handy)( - |:)?|Telefon(ino|o)?( per anziani)?|mobile |(leicht)? gebraucht|(Micro-|Neuwertiges |Komfort-)?(Handy|Natel) (von |\/ |- )?|Handy\s*/\s*(Natel|Smart(fon|phone))( - |:)?|zu verkaufen ?(ein )?|Verkauf von |vendo |originalverpackt|neu & ungeöffnet|(in|mit|NEU und| und)? OVP|( - )?(wie )?neu(es?|wertig)?| und noch verschweisst| nie benutzt| in gute[mn] Zustand| mit Gebrauchsspuren|einwandfrei|renoviert|4 Farben|Gratisversand|in Lederetui|mit Eingabestift|läuft einwandfrei| MIT GOOGLE SERVICES|(neues |Android )?Smart(fon|phon)e?( -|:)? |Burnerphone |Neuwertiges |Nuovo( |!)| garandieschein| con vetro da sostituire| HD\+|( - )?dual[ -]sim|(\d\.)?\d Zoll|miui|Firmengerät| Apple| ESR| MagSafe| Original Taptic Engine| Original Front Kamera Module| Original Kamera Module| 4K Kamera| Gehäuse Original|Google Sperre|SIMLOCKED| Speicher|condizioni ottime| modell|cellulare |\[DANNEGGIATO\]| RED$| (Android|EU)$`).ReplaceAllString(name, "")

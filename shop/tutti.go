...
var TuttiCleanFn = func(name string) string {
	name = regexp.MustCompile(`(?i)^Original | Entsperrt| Occass?ion| Schnäppchen| GÜNSTIG|Cellulate |funktioniert|Garanzia|RESERVIERT|Top Zustand|funktionstüchtig|im sehr guten Zustand|mit Box|( - )?sehr guter Zustand| in ottimo stato|semplificato |Mobile Phone( - )?|Mobile?telefon | Smartphone| Handy( - )?|Telefon(ino|o)?|mobile |(leicht)? gebraucht|(Micro-|Neuwertiges |Komfort-)?(Handy|Natel) (von |\/ |- )?|Handy\s*/\s*(Natel|Smartphone)|zu verkaufen ?(ein )?|Verkauf von |vendo |originalverpackt|neu & ungeöffnet|(in|mit|NEU und| und)? OVP|( - )?(wie )?neu(es?|wertig)?| und noch verschweisst| nie benutzt| in gute[mn] Zustand| mit Gebrauchsspuren|einwandfrei|renoviert|4 Farben|Gratisversand|in Lederetui|mit Eingabestift|läuft einwandfrei| MIT GOOGLE SERVICES|(neues |Android )?Smartphone? |Burnerphone |Neuwertiges |Nuovo( |!)| garandieschein| con vetro da sostituire| HD\+|( - )?dual[ -]sim|\d\.\d Zoll|miui|Firmengerät| Apple| Original Taptic Engine| Original Front Kamera Module| Original Kamera Module| Gehäuse Original|Google Sperre|SIMLOCKED| Speicher|condizioni ottime| modell|cellulare |\[DANNEGGIATO\]| RED$| (Android|EU)$`).ReplaceAllString(name, "")
	...
	name = strings.NewReplacer("prima generazione", "1. Gen.", "1Gen  Rigenerato", "1. Gen.", "1 Generation", "1. Gen.", " G5G", " G", " 2GB", "", "20 e", "20e", "FE20", "S20 FE", "A5-6", "A5", "Galxy", "Galaxy", "XSMax", "XS Max", "Mate-20", "Mate 20", "Motorolla", "Motorola", "Sansung", "Samsung", "SAMSUG", "SAMSUNG", "Galaxie", "Galaxy", " Tablet", " Tab", "Xcovet", "XCover", "2 Stück", "", "Android", "", "n.201", "", "  ", " ").Replace(name)

...
var TuttiCleanFn = func(name string) string {
	name = regexp.MustCompile(`(?i)^Original | Entsperrt| Occass?ion| Schnäppchen|Cellulate |funktioniert|Garanzia|RESERVIERT|Top Zustand|( - )?sehr guter Zustand| in ottimo stato|semplificato |Mobile Phone( - )?|Mobile?telefon | Smartphone| Handy( - )?|Telefon(ino)?|mobile |(leicht)? gebraucht|(Micro-|Neuwertiges )?(Handy|Natel) (von |\/ |- )?|Handy/Natel|zu verkaufen ?(ein )?|Verkauf von |vendo |originalverpackt|(in|mit|NEU und| und)? OVP|( - )?(wie )?neu(e|wertig)?| und noch verschweisst| in gutem Zustand| mit Gebrauchsspuren|einwandfrei|renoviert|4 Farben|Gratisversand|in Lederetui|mit Eingabestift|läuft einwandfrei| MIT GOOGLE SERVICES|(neues |Android )?Smartphone? |Burnerphone |Neuwertiges | garandieschein| con vetro da sostituire| HD\+|( - )?dual[ -]sim|miui|Firmengerät| Apple| Original Taptic Engine| Original Front Kamera Module| Original Kamera Module| Gehäuse Original|SIMLOCKED|condizioni ottime|cellulare |\[DANNEGGIATO\]| RED$| (Android|EU)$`).ReplaceAllString(name, "")

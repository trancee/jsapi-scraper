...
var TuttiRegex = regexp.MustCompile(`(?i)[,-]? ?[\[(]?\b(2|3|4|6|8|12|16|32|54|64|65|128|250|256|265|512)[ .\/]?([MG][BG]|BG|G|B\b|\+)[)\]]?( RAM)?|(\. )?\b(6|8|12|128|256)(GB)?\/(6|8|12|128|256)(GB)?\b|\/6\s+|\(\d\.\d{1,2} "| \d(\.\d{1,2})?"|\d+ mm|-?[ (\/]?[45] ?G[)]?| GSM| LTE| inkl\. | mit | und | and more| [*|] | \([^\d]| \/|, |\/ | -|;`)
...
var TuttiCleanFn = func(name string) string {
	name = regexp.MustCompile(`(?i)^Original | - neuwertiger Zustand|^\*\*\*\*|^\*\*|^\+\+ | \+\+$| \+ Airpods| Entsperrt|^Für Einsteiger: |^Neu: |^Zu verkaufen: |Supper | Occass?ion|Gebrauchtes | Schnäppchen| GÜNSTIG|>BILLIG: |Hochwertiges |Cellulate |funktioniert (100%)?|bisschen kaputt aber geht noch gut| \d+%( Battery| di batteria)?|Batteriezustand|Batteriekapazität( \d+%)?|COMPLETO FUNZIONA BENISSIMO|(Perfetto |Ricondizionato )?Garanzia( 12Mesi)?( n\.\d{3})?|RESERVIERT|(guter?|in (einem )?sehr gute[nm]|in exzellente[nm]|neuwertiger|Perfekter|(in )?Top|Super) Zustand|ohne Kratzer|(voll )?funktionstüchtig|im sehr guten Zustand|mit Box|( - )?sehr guter Zustand|Hammer Geil| in ottimo stato|semplificato |Mobile Phone( - )?|Mobile?telefon (von )?| (Smart(fon|phone)|Handy)( - |:)?|Telefon(ino|o)?( per anziani)?|mobile |(leicht)? gebraucht( & gelöscht)?|(Micro-|Neuwertiges |Komfort-)?(Handy|Natel) (von |\/ |- )?|Neuwertiges |Handy\s*/\s*(Natel|Smart(fon|phone))( - |:)?|(zu )?verkaufen? ?(ein )?|Verkauf von |vendo | vendesi|preis verhandelbar|\(Tausche\/Verhandelbar\) |originalverpackt|neu & ungeöffnet|(in|inklusive|mit|NEU und| und)? OVP|( - )?(fast |wie |Fabrik)?neu(es?|wertig)?|24 Monate Garantie| New Version| und noch verschweisst| nie benutzt| in gute[mn] Zustand| mit Gebrauchsspuren|einwandfrei|renoviert|4 Farben|Gratis ?versand|in Lederetui|mit Eingabestift|läuft einwandfrei| MIT GOOGLE SERVICES|Original OS|CalyxOS|GrapheneOS|(neues |Android )?Smart(fon|phon)e?( -|:)? |Burnerphone |Neuwertiges |in usato|(Usato ma )?(Come |Praticamente )?Nuovo( |!)?| garandieschein| anno| année| con vetro da sostituire| \+ accessori| Glasschaden| HD\+|( - )?dual[ -]?sim( free)?|(\d\.)?\d Zoll|miui|Firmengerät| Apple| ESR| MagSafe| Original Taptic Engine| Original Front Kamera Module| Original Kamera Module| 4K Kamera| inkl\. Ladegerät| Gehäuse Original|Google Sperre|SIMLOCKED| Speicher| di Xiaomi|condizioni ottime| modell|cellulare |\[DANNEGGIATO\]|4\/128(GB)?|(12|16|32|64|128|256)\+(2|3|4|6|8|12|16)GB|(2|3|4|6|8|12|16)\s*ram|(16|32|64|128|256)\s*rom|512,$|Breathing Crystal|Red Product| RED$|Smaragdgrün|Bitte lesen!|\s*!*$| (Android|EU)$`).ReplaceAllString(name, "")
	...
	name = strings.NewReplacer("prima generazione", "1. Gen.", "1Gen  Rigenerato", "1. Gen.", "1 Generation", "1. Gen.", "1st. Generation", "1. Gen.", "1Gen", "1. Gen.", "1 Gen", "1. Gen.", "1 gen", "1. Gen.", "Gen 1", "1. Gen.", "primoo", "1. Gen.", "SE1", "SE 1. Gen.", "Redmy", "Redmi", " G5G", " G", " 2GB", "", "10 e", "10e", "20 e", "20e", "FE20", "S20 FE", "S21FE", "S21 FE", "A5-6", "A5", "Galxy", "Galaxy", "XSMax", "XS Max", "Mate-20", "Mate 20", "Motorolla", "Motorola", "Sansung", "Samsung", "SAMSUG", "SAMSUNG", "Galaxie", "Galaxy", "S10 DUOS S10 NORMAL", "S10 DUOS", " Tablet", " Tab", "Xcovet", "XCover", "2 Stück", "", "2 Stuck", "", "Android", "", "n.201", "", "Litle", "Lite", "ß", "ss", "|", " ", "  ", " ").Replace(name)

...
func (s *traverseState) render(buf *bytes.Buffer, ptrs int, v reflect.Value, implicit bool) {
	...
	case "*timestamppb.Timestamp":
		t := ptypes.TimestampString(v.Interface().(*timestamppb.Timestamp))
		buf.WriteRune('"')
		buf.WriteString(t)
		buf.WriteRune('"')
		return
	...
	case reflect.Struct:
		...
		for i := 0; i < vt.NumField(); i++ {
			// FIXME: Skip field name starting with XXX.
			if strings.HasPrefix(vt.Field(i).Name, "XXX_") {
				continue
			}
			// FIXME: Do not include any fields from external package.
	...
	default:
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			// FIXME: Using %v allows to show enum string instead of value.
			fmt.Fprintf(buf, "%v", v)
